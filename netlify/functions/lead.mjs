...
async function sendTelegram(config, lead) {
  ...
  if (missing.length) {
    throw new Error(`не найдены переменные ${missing.join(', ')}`);
  }
  ...
  if (!apiResponse.ok || !result.ok) {
    throw new Error(
      `Telegram API ${apiResponse.status}: ${result.description || 'запрос отклонён'}`,
    );
  }
}
...
async function sendEmail(config, lead) {
  ...
  if (!config.emailTo) missing.push('LEAD_EMAIL_TO');
...
export default async (request) => {
  ...
  if (
    ...
    return json(400, {
      ok: false,
      message: 'Проверьте заполнение полей и согласие на обработку данных.',
    });
  }
  ...
  const telegramError = telegramOk
    ? null
    : safeError(telegramResult.reason);

  const emailError = emailOk
    ? null
    : safeError(emailResult.reason);
  ...
  if (!telegramOk && !emailOk) {
    return json(502, {
      ok: false,
      message:
        `Telegram: ${telegramError}. Почта: ${emailError}.`,
    });
  }
